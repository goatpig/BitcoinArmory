...
class DlgLockboxEditor(ArmoryDialog):
   ...
   def __init__(self, parent, main, maxM=LB_MAXM, maxN=LB_MAXN, loadBox=None):
      ...
      for i in range(self.maxN):
         ...
         self.widgetMap[i]['LBL_WLTN'] = QRichLabel( \
            self.tr('Name or ID:'), \
            doWrap=False, \
...
#                 isValid = False
      ...
      self.lockbox = MultiSigLockbox( str(self.edtBoxName.text()),
                                      str(self.longDescr),
                                      currM,
                                      currN,
                                      dPubKeys,
...
class DlgLockboxManager(ArmoryDialog):
   def __init__(self, parent, main):
      ...
      initialColResize(self.ledgerView, [
         cWidth, 0, dateWidth, tWidth,
         0.30, 0.40, 0.3
      ])

      self.ledgerView.doubleClicked.clicked(self.dblClickLedger)
   ...
   def getDisplayRichText(self, lb, tr=None, dateFmt=None):
      ...
      shortName = str(lb.shortName)
      if len(shortName.strip())==0:
         shortName = u'<No Lockbox Name'

      longDescr = str(lb.longDescr)
...
class DlgExportAsciiBlock(ArmoryDialog):
   def __init__(self, parent, main, exportObj, title, descr, ftypes, defaultFN):
      ...
      frmCopy = makeHorizFrame([
         btnSave, btnCopy, btnMail,
         self.lblCopyMail, 'Stretch'
      ])
      frmDone = makeHorizFrame(['Stretch', btnDone])
   ...
   def savefile(self):
      fn = self.main.getFileSave(self.tr('Export ASCII Block'),
         self.fileTypes,
         self.defaultFN)
      if fn:
         with open(fn,'w') as f:
   ...
   def doSignForInput(self, idStr, keyIdx):
      ...
      try:
         if ib.lockbox:
            ...
            #HighPrioTODO: Replace with cpp bridge
